        ...
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-7 grid sm:grid-cols-2 gap-4">
            {PILLARS.map((p, i) => (
              <div
                key={p.title}
                className="rounded-2xl bg-white border border-navy-100 p-7 hover:shadow-card hover:-translate-y-1 transition animate-fade-up"
                style={{ animationDelay: `${i * 0.08}s` }}
              >
                <div className="w-11 h-11 rounded-xl bg-brand-orangeSoft text-brand-orangeDark flex items-center justify-center">
                  <p.icon />
                </div>
            ...
            <div className="rounded-2xl bg-brand-navy text-white p-7 sm:col-span-2 flex flex-col sm:flex-row sm:items-center justify-between gap-5 relative overflow-hidden">
              <div
                className="absolute -right-20 -bottom-20 w-56 h-56 rounded-full bg-brand-orange/25 blur-3xl animate-pulse-soft"
                aria-hidden
              />
